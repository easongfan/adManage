...
export async function DELETE(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (prismaClient) => {
      // 返还客户余额
      const currentBalance = customer.balance ? Number(customer.balance) : 0;
      const spendAmount = spendLog.amount ? Number(spendLog.amount) : 0;
      
      const updatedCustomer = await prismaClient.user.update({
        where: { id: customerId },
        data: {
          balance: currentBalance + spendAmount,
        },
      });
